...
class HIDDigitizerService:
    ...
    def handle_command(self, command_str: str) -> None:
        ...
        try:
            ...
            elif cmd_type == CMD_MEDIA_PLAY_PAUSE:
                self.logger.info("Executing MEDIA_PLAY_PAUSE")
                self.hid.play_pause()
                self.serial.send_response(CommandFormatter.format_response(True))

            elif cmd_type == CMD_MEDIA_NEXT:
                self.logger.info("Executing MEDIA_NEXT")
                self.hid.next_track()
                self.serial.send_response(CommandFormatter.format_response(True))

            elif cmd_type == CMD_MEDIA_PREV:
                self.logger.info("Executing MEDIA_PREV")
                self.hid.prev_track()
                self.serial.send_response(CommandFormatter.format_response(True))

            elif cmd_type == CMD_BUTTON_PRESS:
                button = params["button"]
                self.logger.info(f"Executing BUTTON_PRESS {button}")
                # Set button bit based on button type
                if button == "left":
                    self.hid.current_buttons |= BUTTON_TIP_SWITCH
                elif button == "right":
                    self.hid.current_buttons |= BUTTON_BARREL
                # Send HID report with updated button state
            ...
            elif cmd_type == CMD_BUTTON_RELEASE:
                button = params["button"]
                self.logger.info(f"Executing BUTTON_RELEASE {button}")
                # Clear button bit based on button type
                if button == "left":
                    self.hid.current_buttons &= ~BUTTON_TIP_SWITCH
                elif button == "right":
                    self.hid.current_buttons &= ~BUTTON_BARREL
